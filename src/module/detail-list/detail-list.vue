<template>
  ...
  <ul class="detail-list">
    <li class="list-item" v-for="(item, index) in detailList" :key="index">
      ...
      <div class="item-default" :class="{'border-bottom-1': item.border}" v-else-if="item.type == 'default'">
        <p>{{item.key}}</p>
        <p>{{item.value}}</p>
      </div>
      <div class="item-total border-bottom-1" v-else-if="item.type == 'total'">
        <div class="total-content">
          ...
          <div class="content-balance color-light-grey font-24">
            <p class="balance-item margin-right-24">
              <span>支出</span>
              <span>¥{{item.pay}}</span>
            </p>
      ...
      <div class="item-double padding-left-30" v-else-if="item.type == 'double'" @click="doubleEvent(item.oid)">
        <div class="item-content border-bottom-1">
          <div class="content-tip">
            ...
            <p class="font-30">{{item.count}}</p>
          </div>
          <div class="content-tip color-light-grey font-24">
            <p>{{item.time}}</p>
            <p>{{item.remnant}}</p>
          </div>
        </div>
