<template>
  ...
  <section class="pull-refresh" @touchstart="handleTouchStart($event)" @touchmove="handleTouchMove($event)" @touchend="handleTouchEnd($event)">
    ...
    <div class="refresh-list" :style="{top: direction == 'top' ? `${tipHeight}px` : `${-tipHeight}px`}">
      <slot></slot>
    </div>
  ...
  methods: {
    ...
    handleTouchMove (event) {
      ...
      if (this.direction === 'top') {
        ...
        if (this.parentDom.scrollTop > 0) return
        this.tip = '下拉加载'
